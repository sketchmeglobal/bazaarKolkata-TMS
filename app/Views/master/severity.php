    ...
    <div class="container-xxl position-relative bg-white d-flex p-0">
        ...
        <div class="content">
                ...
                <div class="container-fluid">
                      <nav aria-label="breadcrumb" class="row bg-breadcrumb">
                        <ol class="breadcrumb my-0 ms-2">
                          ...
                          <li class="breadcrumb-item active"><span>Severity</span></li>
                        </ol>
                      </nav>
                    </div>
                </div>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="d-flex justify-content-evenly">
                                        <a href="javascript: void(0);" class="edit_class" data-table_id="<?=$row['ticket_severity_id']?>"><i class="fa fa-edit"></i></a>
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="text-primary modal-title" id="exampleModalLongTitle">Severity</h5>
                            ...
                            <form class="needs-validation" id="s_myFormName" method="post">
                                <div class="form-row row">
                                    ...
                                    <div class="col-md-4 mb-1">
                                        <label for="ticket_severity_name">Severity Name</label>
                                        <input type="text" class="form-control" name="ticket_severity_name" id="ticket_severity_name" required
                                            value="<?= isset($name) ? $name : '' ?>">
                                        <span class="error" id="ticket_severity_nameError">
                                            <?=(isset($validation['name']) ? $validation['name'] : '' ); ?>
                                        </span>
                                    </div>
                                    <div class="col-md-4 mb-1">
                                        <label for="max_allowed_time">Max. Allowed Time(Hour)</label>
                                        <input minlength="5" type="text" class="form-control" name="max_allowed_time"
                                            id="max_allowed_time" required
                                            value="<?= isset($address) ? $address : '' ?>">
                                        <span class="error" id="max_allowed_timeError"><?= (isset($validation['address']) ? $validation['address'] : ''); ?></span>
                                    </div>
            ...
            function validateForm(){
                $ticket_severity_name = $('#ticket_severity_name').val().replace(/^\s+|\s+$/gm,'');
                $max_allowed_time = $('#max_allowed_time').val().replace(/^\s+|\s+$/gm,'');
                
                $status = true;
                $formValidMsg = '';
                
                if($ticket_severity_name == ''){
                    $status = false;
                    $formValidMsg += 'Please enter severity name';
                    $('#ticket_severity_name').removeClass('is-valid');
                    $('#ticket_severity_name').addClass('is-invalid');
                }else{
                    $('#ticket_severity_name').removeClass('is-invalid');
                    $('#ticket_severity_name').addClass('is-valid');
                }

                if($max_allowed_time == ''){
                    $status = false;
                    $formValidMsg += ', max allowed time';
                    $('#max_allowed_time').removeClass('is-valid');
                    $('#max_allowed_time').addClass('is-invalid');
                }else{
                    $('#max_allowed_time').removeClass('is-invalid');
                    $('#max_allowed_time').addClass('is-valid');
                } 
            ...
            $('#s_submitForm').click(function(){
                ...
                setTimeout(function(){
                    ...
                    if($formVallidStatus == true){
                        ...
                        $.ajax({  
                            url: '<?php echo base_url('admin/formValidationSE'); ?>',
                            type: 'post',
                            dataType:'json',
                            data:{query: $query},
                            success:function(data){
                                ...
                                if(data.status == true ){
                                    $('#ticket_severity_nameError').html('');
                                    $('#max_allowed_timeError').html('');
                                    ...
                                    if(parseInt(data.ho_id) > 0){
                                        ...
                                        var row = $('<tr>')
                                            ...
                                            .append('<td>'+$ticket_severity_name+'</td>')
                                            .append('<td>'+$max_allowed_time+'</td>')
            ...
            $(".remove").click(function() {
                if(confirm("Are You Sure? This Process Can\'t be Undone.")){
                    ...
                    $.ajax({  
                        url: '<?php echo base_url('admin/removeTableDataSE'); ?>',
                        type: 'post',
                        dataType:'json',
            ...
            $(".edit_class").click(function() {
                ...
                $.ajax({  
                    url: '<?php echo base_url('admin/getTableDataSE'); ?>',
                    type: 'post',
                    dataType:'json',
                    data:{table_id: $table_id},
                    success:function(data){
                        ...
                        //console.log('status: ' + data.status);
                        if(data.status == true ){
                            $('#ticket_severity_name').val(data.result.ticket_severity_name);
                            $('#max_allowed_time').val(data.result.max_allowed_time);
                            $('#table_id').val(data.result.ticket_severity_id);
                            $('#myModal').modal('show');
                        }
                    }  
                });//end ajak
