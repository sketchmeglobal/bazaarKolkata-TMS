...
<body>
                    ...
                    <div class="modal-dialog modal-lg" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLongTitle"> Add/Edit Warehouse name</h5>
                                ...
                                <form class="needs-validation" novalidate name="s_myFormName" id="s_myFormName">
                                    <div class="form-row">
                                        <div class="col-md-4 mb-1">
                                            ...
                                            <input type="text" class="form-control" name="warehouseName" id="warehouseName" value="" > 
                                        </div>  
                                        <div class="col-md-4 mb-1">
                                            <label for="warehouseLocation">Warehouse Location</label>
                                            <input type="text" class="form-control" name="warehouseLocation" id="warehouseLocation" value="" > 
                                        </div>                              
    ...
    <script>
        ...
        function validateForm(){
            $warehouseName = $('#warehouseName').val().replace(/^\s+|\s+$/gm,'');
            $warehouseLocation = $('#warehouseLocation').val().replace(/^\s+|\s+$/gm,'');
            ...
            if($warehouseName == ''){
                $status = false;
                $formValidMsg += 'Please enter Warehouse name';
                $('#warehouseName').removeClass('is-valid');
                $('#warehouseName').addClass('is-invalid');
            }else{
                $('#warehouseName').removeClass('is-invalid');
                $('#warehouseName').addClass('is-valid');
            }

            if($warehouseLocation == ''){
                $status = false;
                $formValidMsg += ', location';
                $('#warehouseLocation').removeClass('is-valid');
                $('#warehouseLocation').addClass('is-invalid');
            }else{
                $('#warehouseLocation').removeClass('is-invalid');
                $('#warehouseLocation').addClass('is-valid');
            } 
        ...
        $('#myTable').on('click', '.edit_class', function(){ 
            ...
            $('#warehouseName').val('Baazar Kolkata');
            $('#warehouseLocation').val('Newtown');
            $('#myModal').modal('show');
