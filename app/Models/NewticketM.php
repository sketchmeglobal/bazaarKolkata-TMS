...
class NewticketM extends Model
{
    ...
    protected $allowedFields    = ['ticket_id','topic_id', 'ticket_subject', 'ticket_category', 'ticket_severity', 'authority_cc', 'ticket_purpose', 'ticket_description', 'ticket_status'];
    ...
    public function getTopicMaster(){
      $topic_rows = $this->db->table('topic_master')->select('*')->where(['row_status' => 1])->get()->getResult();      
      return $topic_rows;
    }//end function

    public function getCategoryMaster(){
      $category_rows = $this->db->table('ticket_category_master')->select('*')->where(['row_status' => 1])->get()->getResult();
